...
1. **Contact Class**  
   - Properties: id, first\_name, last\_name, phone\_number (will be referred to as **`<Contact Params>`**)  
   - Method to convert contact to dictionary format

   

2. **Database Service Class with the following Functions (Interacts with the database):**  
   - `create_contact(Dict: <Contact Params>)` → returns new contact ID (queried from DB) \=\> Create a new contact (INSERT)  
   - `get_all_contacts()` → returns list of Contact objects \=\> Retrieve all contacts (SELECT)  
   - `update_contact(id, Dict: <Contact Params>)` → returns success boolean \=\> Update a contact (UPDATE)  
   - `delete_contact(id)` → returns success boolean \=\> Delete a contact (DELETE)  
       
     Note: SQL queries should be written manually using raw SQL statements (formatted to include relevant parameters).

3. **Database Module (Optional \- can be implemented in the database service)**  
